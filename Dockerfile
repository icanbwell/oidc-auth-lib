...
COPY Pipfile* /usr/src/oidcauthlib/
...
COPY --from=python_packages /usr/src/oidcauthlib/Pipfile.lock /usr/src/oidcauthlib/Pipfile.lock

# Copy Pipfile.lock to a temporary directory so it can be retrieved if needed
COPY --from=python_packages /usr/src/oidcauthlib/Pipfile.lock /tmp/Pipfile.lock
