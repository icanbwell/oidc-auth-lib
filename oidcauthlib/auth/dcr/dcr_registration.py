...
class DcrRegistration(BaseDbModel):
    ...
    updated: Optional[datetime] = Field(
        default=None, description="When the registration was last updated."
    )
    ...
    client_secret: Optional[str] = Field(
        default=None, description="The client_secret from DCR (if any)."
    )
    client_secret_expires_at: int = Field(
        default=0,
        description="Unix timestamp when client_secret expires. 0 = no expiry.",
    )
    registration_response: dict[str, Any] = Field(
        default_factory=dict, description="The full DCR response."
    )
