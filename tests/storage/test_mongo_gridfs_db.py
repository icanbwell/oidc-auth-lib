...
async def test_inline_store_and_get_roundtrip(test_container: IContainer) -> None:
    ...
    test_container.singleton(
        ...
        lambda c: CacheToCollectionMapperTester(environment_variables=c.resolve(OidcEnvironmentVariables)),
    )
    ...
    entry = ManagedEntry(value={"hello": "world"}, created_at=datetime.now(UTC), expires_at=None)
    await store._put_managed_entry(key="k_inline", collection="gridfs_inline_test", managed_entry=entry)
    got = await store._get_managed_entry(key="k_inline", collection="gridfs_inline_test")
...
async def test_gridfs_store_and_get_roundtrip(test_container: IContainer) -> None:
    ...
    await store._put_managed_entry(key="k_big", collection="gridfs_big_test", managed_entry=entry)
...
async def test_update_cleanup_old_gridfs_file(test_container: IContainer) -> None:
    ...
    entry1 = ManagedEntry(value=large_obj, created_at=datetime.now(UTC), expires_at=None)
    await store._put_managed_entry(key="k_update", collection="gridfs_update_test", managed_entry=entry1)

    # capture existing file id from metadata
    metadata_doc = await store._collections_by_name["gridfs_update_test"].find_one(filter={"key": "k_update"})
    ...
    small_obj = {"ok": True}
    entry2 = ManagedEntry(value=small_obj, created_at=datetime.now(UTC), expires_at=None)
    await store._put_managed_entry(key="k_update", collection="gridfs_update_test", managed_entry=entry2)

    # metadata should now have inline_value and old gridfs id removed
    metadata_doc2 = await store._collections_by_name["gridfs_update_test"].find_one(filter={"key": "k_update"})
    assert metadata_doc2 is not None
    assert "inline_value" in metadata_doc2
    assert "gridfs_file_id" not in metadata_doc2 or metadata_doc2.get("gridfs_file_id") is None

    got2 = await store._get_managed_entry(key="k_update", collection="gridfs_update_test")
...
async def test_batch_get_mixed_inline_and_gridfs(test_container: IContainer) -> None:
    ...
    large = ManagedEntry(value={"l": "z" * 5000}, created_at=datetime.now(UTC), expires_at=None)

    await store._put_managed_entry(key="k_small", collection="gridfs_batch_test", managed_entry=small)
    await store._put_managed_entry(key="k_large", collection="gridfs_batch_test", managed_entry=large)

    # Batch get
    results = await store._get_managed_entries(collection="gridfs_batch_test", keys=["k_small", "k_large", "k_missing"])
...
async def test_batch_get_all_missing_returns_nones(test_container: IContainer) -> None:
    ...
    keys = ["na1", "na2", "na3"]
    results = await store._get_managed_entries(collection="gridfs_all_missing_test", keys=keys)
    assert len(results) == 3
...
async def test_delete_managed_entries_bulk(test_container: IContainer) -> None:
    ...
    e2 = ManagedEntry(value={"b": "q" * 5000}, created_at=datetime.now(UTC), expires_at=None)
    e3 = ManagedEntry(value={"c": 3}, created_at=datetime.now(UTC), expires_at=None)

    await store._put_managed_entry(key="a", collection="gridfs_delete_bulk", managed_entry=e1)
    await store._put_managed_entry(key="b", collection="gridfs_delete_bulk", managed_entry=e2)
    await store._put_managed_entry(key="c", collection="gridfs_delete_bulk", managed_entry=e3)

    # Delete two keys
    deleted = await store._delete_managed_entries(keys=["a", "b"], collection="gridfs_delete_bulk")
    assert deleted >= 2
...
async def test_delete_managed_entry_single_path(test_container: IContainer) -> None:
    ...
    store.max_inline_size_kb = 1
    e = ManagedEntry(value={"one": "value"}, created_at=datetime.now(UTC), expires_at=None)
    await store._put_managed_entry(key="one", collection="gridfs_delete_single", managed_entry=e)

    # Ensure exists
    assert await store._get_managed_entry(key="one", collection="gridfs_delete_single") is not None

    # Delete single
    ok = await store._delete_managed_entry(key="one", collection="gridfs_delete_single")
    assert ok is True
    # Verify gone
    assert await store._get_managed_entry(key="one", collection="gridfs_delete_single") is None

    # Delete non-existent
    ok2 = await store._delete_managed_entry(key="missing", collection="gridfs_delete_single")
    assert ok2 is False
...
async def test_gridfs_stats_and_delete_collection(test_container: IContainer) -> None:
    ...
    store.max_inline_size_kb = 1
    e1 = ManagedEntry(value={"x": "m" * 3000}, created_at=datetime.now(UTC), expires_at=None)
    e2 = ManagedEntry(value={"y": "n" * 4000}, created_at=datetime.now(UTC), expires_at=None)
    await store._put_managed_entry(key="x", collection="gridfs_stats_test", managed_entry=e1)
    await store._put_managed_entry(key="y", collection="gridfs_stats_test", managed_entry=e2)
    ...
    assert "chunk_size_bytes" in stats and stats["chunk_size_bytes"] == store._gridfs_chunk_size_kb * 1024
    ...
    await store.setup_collection(collection="gridfs_stats_test")
    assert await store._get_managed_entry(key="x", collection="gridfs_stats_test") is None
    assert await store._get_managed_entry(key="y", collection="gridfs_stats_test") is None
